...
export const userResolvers = {
 Query: {
  ...
  get_recommended_users: async ( _: any,{query}: {query: string}, context: any) => {
   const {id} = context.user;
  ...
  get_user_data: async (_: any,__: any,context: any) => {
   ...
   try {
    const user = await sql.execute(`SELECT * FROM users WHERE id = ?`,[id]) as any;
    return user[0][0];
   } catch(error) {
    if(error instanceof Error) throw new GraphQLError(error.message);
   }
 ...
 Mutation: {
  update_user: async (_: any,args: any,context: any) => {
   ...
   try {
    ...
    let user_query = `SELECT name, email, username, profile, cover, bio FROM users WHERE id = ?`;

    const user = await sql.execute(user_query, [id]) as any;
